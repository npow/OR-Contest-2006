<p><pre>
<?php readfile("sample_data"); ?>
</pre></p>
<hr />
<p><pre>
<?php readfile("data/ORcontestData.csv"); ?>
</pre></p>
<hr />
<p><pre>
<?php
$fp = fopen("data/ORcontestData.csv", "r");
...
for($i = 0; $i < sizeof($list); $i++) {
	...
	if($list[$i] <= $ms[$level]) {
		...
		if($level == 0) {
			$sec[1] .= "\n\ta(j)\t'number for small output'\t/".($i-$ctm+1)."*".($i+1)."/";
			$level += 2;
		}
		else if($level == 2) {
			$sec[1] .= "\n\tb(j)\t'number for medium output'\t/".($i-$ctm+1)."*".($i+1)."/";
			$level++;
		} else {
			$sec[1] .= "\n\tk(j)\t'number for large output'\t/".($i-$ctm+1)."*".($i+1)."/";
		}
	}
}
fclose($fp);
